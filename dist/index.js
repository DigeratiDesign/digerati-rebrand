...
(() => {
  ...
  // src/vendor/breathing-halftone.ts
  init_live_reload();

  // src/vendor/vector.ts
  init_live_reload();
  ...
  var Particle = class {
    constructor(properties) {
      ...
      this.position = vector_default.copy(this.origin);
      this.velocity = new vector_default();
      this.acceleration = new vector_default();
    ...
    calculateSize() {
      ...
      let oscSize = now / (1e3 * opts.oscPeriod) * TAU;
    ...
    getChannelValue() {
      ...
      if (opts.isChannelLens) {
        ...
        if (this.originChannelValue === void 0) {
    ...
    applyOriginAttraction() {
      const attraction = vector_default.subtract(this.position, this.origin);
  ...
  var supports = { canvas: false, darker: false };
  ...
  function BreathingHalftone(img, options = {}) {
    const defaults = {
      dotSize: 1 / 40,
    ...
    console.log("Canvas created and inserted:", canvas);
    ...
    const getImgData = () => {
      ...
      const ctx2 = canvasAndCtx.ctx;
      imgWidth = imgCanvas.width = img.naturalWidth;
      imgHeight = imgCanvas.height = img.naturalHeight;
      ctx2.drawImage(img, 0, 0);
      imgData = ctx2.getImageData(0, 0, imgWidth, imgHeight).data;
    };
    const resizeCanvas = () => {
      const w = canvas.width = img.offsetWidth;
      const h = canvas.height = img.offsetHeight;
      const diagonal2 = Math.sqrt(w * w + h * h);
      const imgScale2 = w / imgWidth;
      const gridSize2 = optionsMerged.dotSize * diagonal2;
    ...
    const initParticles = () => {
      ...
      for (const channel of channels) {
        ...
        channelParticles2[channel] = channelParticlesArray;
        particles2 = particles2.concat(channelParticlesArray);
      }
    };
    const animate = () => {
      update();
    ...
    const update = () => {
      for (const particle of particles) {
        for (const identifier in cursors) {
          ...
          const force = vector_default.subtract(particle.position, cursor.position);
    ...
    const renderGrid = (channel) => {
      ...
      const particles2 = channelParticles[channel];
      for (const particle of particles2) {
        particle.render(proxy.ctx);
      }
      ctx.drawImage(proxy.canvas, 0, 0);
    };
    const getCartesianGridParticles = (channel, angle) => {
      const particles2 = [];
      const w = canvas.width;
      const h = canvas.height;
      const diag = Math.max(w, h) * ROOT_2;
      const gridSize2 = optionsMerged.dotSize * diag;
      const cols = Math.ceil(diag / gridSize2);
      const rows = Math.ceil(diag / gridSize2);
      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
          let x1 = (col + 0.5) * gridSize2;
          let y1 = (row + 0.5) * gridSize2;
          x1 -= (diag - w) / 2;
          y1 -= (diag - h) / 2;
          x1 -= w / 2;
          y1 -= h / 2;
    ...
    const getRadialGridParticles = (channel, angle) => {
      const particles2 = [];
      const w = canvas.width;
      const h = canvas.height;
      const diag = Math.max(w, h) * ROOT_2;
      const gridSize2 = optionsMerged.dotSize;
      const halfW = w / 2;
      const halfH = h / 2;
      const offset = gridSize2;
      const centerX = halfW + Math.cos(angle) * offset;
      const centerY = halfH + Math.sin(angle) * offset;
      const maxLevel = Math.ceil((diag + offset) / gridSize2);
      for (let level = 0; level < maxLevel; level++) {
        const max = level * 6 || 1;
        for (let j = 0; j < max; j++) {
          const theta = TAU2 * j / max + angle;
          const x = centerX + Math.cos(theta) * level * gridSize2;
          const y = centerY + Math.sin(theta) * level * gridSize2;
  ...
  // src/digerati/convertMarkdownToTable.ts
  init_live_reload();
  var import_showdown = __toESM(require_showdown(), 1);
  var convertMarkdownToTable = () => {
