...
(() => {
  ...
  var Particle = class {
    ...
    calculateSize() {
      ...
      let oscSize = now.getTime() / (1e3 * opts.oscPeriod) * TAU;
  ...
  var BreathingHalftone = class _BreathingHalftone {
    constructor(img, options = {}) {
      ...
      this.isActive = false;
      this.options = { ..._BreathingHalftone.defaults, ...options };
      this.img = img;
      if (!supports.canvas)
        return;
      this.create();
    ...
    getCartesianGridParticles(channel, angle) {
      const particles = [];
      const w = this.width;
      const h = this.height;
