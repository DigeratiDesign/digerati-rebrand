...
(() => {
  ...
  var isDevEnv = typeof process !== "undefined" && typeof process.env !== "undefined" && true;
...
`;
  ...
  // src/client/modules/colourCycleTargets.ts
  init_live_reload();
  var ColourCycleTargets = class {
    constructor(options = {}) {
    ...
    init() {
      ...
      this.selectors.forEach((selector3) => {
  ...
  function initLegalColourCycle(options) {
    ...
    autoGroup("Tally Init", () => {
      ...
      const closeModal = () => {
        autoGroup("Close Modal", () => {
          eventBus.emit("tally:close", void 0);
          ...
          eventBus.emit("tally:closed", void 0);
        });
      };
      const onBodyClick = (e) => {
      ...
      eventBus.emit("tally:initialized", void 0);
    });
    return handles;
  };
  ...
  var testimonialAvatar = () => {
    autoGroup("Testimonial Avatar", () => {
      eventBus.emit("testimonialAvatar:init", void 0);
      if (!shouldAnimate()) {
  ...
  var cycleTimer = null;
  function initRandomBrandIcons(opts = {}) {
    const {
      persist = "none",
      cacheBust = true,
      cycle = false
    ...
    const totalPeriodMs = cycle ? cycleCfg.periodMs ?? 12e3 : 0;
    [...WEBCLIPS, ...FAVICONS].forEach((src) => {
      const i = new Image();
      i.src = src;
    });
    ...
    let index = typeof cycleCfg.startIndex === "number" ? clampIndex(cycleCfg.startIndex) : persisted != null && !Number.isNaN(+persisted) ? clampIndex(+persisted) : Math.floor(Math.random() * FAVICONS.length);
    applyIndex(index, links, cacheBust);
    if (store)
      store.setItem(key, String(index));
    if (cycleTimer != null) {
  ...
  function getStore(mode) {
    try {
      return mode === "local" ? localStorage : mode === "session" ? sessionStorage : null;
    } catch {
      return null;
    }
  }
  function ensureIconLinks() {
    const ensure = (rel, sizes, type = "image/png") => {
  ...
  function applyIndex(index, links, cacheBust) {
  ...
  var reasonGenerator = (options = {}) => {
    return autoGroup("Reason Generator", () => {
      ...
      eventBus.emit("reasonGenerator:started", void 0);
      if (!element) {
  ...
  // src/client/constants/blurMessages.ts
  init_live_reload();
  var blurMessages = [
  ...
  var PHASES = {
    ...
    webflowReady: {
      ...
      tasks: [
        ...
        () => initPageBlurTitle({ messages: blurMessages }),
